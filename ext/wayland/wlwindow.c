...
static GstWlWindow *
gst_wl_window_new_internal (GstWlDisplay * display, GMutex * render_lock, struct wl_surface *parent,
    ...
    GMutex * render_lock, gboolean use_subsurface)
{
  ...
  window = gst_wl_window_new_internal (display, render_lock, NULL, use_subsurface);
...
{
  GstWlWindow *window;
  window = gst_wl_window_new_internal (display, render_lock, parent, use_subsurface);
  if (!use_subsurface)
    return window;
